  ...
  >
    <n-input-group>
      <n-input
        ...
        clearable
        :placeholder="t('message.common.tips.searchTips')"
        @input="handleSearch"
      >
        <template #prefix>
      ...
      <n-button v-if="isMobile" type="primary" ghost @click="handleClose">{{ t('message.common.cancel') }}</n-button>
    </n-input-group>

    <div class="mt-20px">
      <n-empty v-if="resultOptions.length === 0" :description="t('message.common.tips.emptyData')" />
...
function search() {
  resultOptions.value = routeStore.searchMenus.filter(menu => {
    const title = menu.meta?.i18nTitle ? t(menu.meta?.i18nTitle) : menu.meta?.title;
