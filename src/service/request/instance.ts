import axios from 'axios';
import type { AxiosResponse, AxiosError, AxiosInstance, AxiosRequestConfig } from 'axios';
import { REFRESH_TOKEN_CODE } from '@/config';
import { useRouterPush } from '@/composables';
import {
  localStg,
  handleAxiosError,
  handleBackendError,
  handleResponseError,
  handleServiceResult,
  transformRequestData
} from '@/utils';
import { handleRefreshToken } from './helpers';

/**
 * 封装axios请求类
 * @author Soybean<honghuangdc@gmail.com>
 */
export default class CustomAxiosInstance {
  instance: AxiosInstance;

  backendConfig: Service.BackendResultConfig;

  /**
   *
   * @param axiosConfig - axios配置
   * @param backendConfig - 后端返回的数据配置
   */
  constructor(
    axiosConfig: AxiosRequestConfig,
    backendConfig: Service.BackendResultConfig = {
      codeKey: 'code',
      dataKey: 'data',
      msgKey: 'message',
      successCode: 0
    }
  ) {
    this.backendConfig = backendConfig;
    this.instance = axios.create(axiosConfig);
    this.setInterceptor();
  }

  /** 设置请求拦截器 */
  setInterceptor() {
    this.instance.interceptors.request.use(
      async config => {
        const handleConfig = { ...config };
        if (handleConfig.headers) {
          // 数据转换
          if (handleConfig.method === 'get') {
            handleConfig.params = handleConfig.data;
          } else {
            const contentType = handleConfig.headers['Content-Type'] as UnionKey.ContentType;
            handleConfig.data = await transformRequestData(handleConfig.data, contentType);
          }
          // 设置token
          handleConfig.headers.Token = localStg.get('token') || '';
          handleConfig.headers.Site = 4;
        }
        return handleConfig;
      },
      (axiosError: AxiosError) => {
        const error = handleAxiosError(axiosError);
        return handleServiceResult(error, null);
      }
    );
    this.instance.interceptors.response.use(
      (async response => {
        const { status, data, request } = response;
        const { responseType } = request;
        if (status === 0) {
          const backend = response.data;
          const { dataKey } = this.backendConfig;
          return handleServiceResult(null, backend[dataKey]);
        }

        if (status === 200 || status < 300 || status === 304 || status === 1000) {
          const backend = response.data;
          const { codeKey, dataKey, successCode } = this.backendConfig;
          // 请求成功
          if (responseType === 'arraybuffer') {
            return handleServiceResult(null, backend);
          }
          if (data.status === 0) {
            return handleServiceResult(null, backend.data);
          }
          if (backend[codeKey] === successCode) {
            return handleServiceResult(null, backend[dataKey]);
          }

          // token失效, 刷新token
          if (REFRESH_TOKEN_CODE.includes(backend[codeKey])) {
            const config = await handleRefreshToken(response.config);
            if (config) {
              return this.instance.request(config);
            }
          }
          // 401鉴权失败, 跳转到登录页面
          if (backend[codeKey] === 401) {
            localStg.set('token', '');
            const { toLogin } = useRouterPush(false);
            toLogin();
          }
          const error = handleBackendError(backend, this.backendConfig);
          Promise.reject(data);
          window.$message?.error(data.message || data.msg);
          return handleServiceResult(error, null);
        }
        const error = handleResponseError(response);
        return handleServiceResult(error, null);
      }) as (response: AxiosResponse<any, any>) => Promise<AxiosResponse<any, any>>,
      (axiosError: AxiosError) => {
        const error = handleAxiosError(axiosError);
        return handleServiceResult(error, null);
      }
    );
  }
}
