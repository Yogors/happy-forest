...
export const useRouteStore = defineStore('route-store', {
  ...
  actions: {
    ...
    handleDone(menuTree: any) {
      for (let index = 0; index < menuTree.length; index += 1) {
        let menu = menuTree[index];
        const { title, url, subMenus, icon } = menu;
    ...
    /** 接口获取权限路由 */
    async initMenu() {
      const { data } = await request.get<any>('/sys/menu/nav');
      return data;
    },
    /** 初始化静态路由 */
    async initStaticRoute() {
      const { initHomeTab } = useTabStore();
      const routesData = await this.initMenu();
      const routes = this.treeDataFormat(routesData);
      this.handleAuthRoute(routes);
      initHomeTab('/dashboard/analysis', router);
      this.isInitAuthRoute = true;
    },

    treeDataFormat(treeJson: any) {
      ...
      treeJson.forEach(item => {
        ...
        if (url && url.startsWith('/')) {
          nameStr = url.substring(1, url.length);
        }
        const componentName = nameStr && nameStr.split('/').join('_');
        let i18nTitle = '';
        const component = item.children && item.children.length > 0 ? 'basic' : 'self';
        if (name.indexOf('i18nTitle:') !== -1) {
          i18nTitle = name.split('i18nTitle:')[1];
        }
        res.push({
          name: componentName,
          path: url,
          ...
          meta: {
            title: name,
            i18nTitle,
            singleLayout: component === 'self' ? 'basic' : 'self',
            icon
          },
          children: children && children.length > 0 ? this.treeDataFormat(children) : []
        });
      });
    ...
    async initEvent() {
      ...
      // this.initDict();
      // await this.initAuthRoute();
    },
    /** 初始化字典 */
    async initDict() {
      const { data } = await request.get<any>('/sys/dict/type/all');
      localStg.set('dictList', data);
    }
  }
});
