<template>
  <div class="w-full text-14px">
    <n-checkbox v-model:checked="checked">{{ t('message.login.agreementTips.tips') }}</n-checkbox>
    <n-button :text="true" type="primary" @click="handleClickProtocol">
      {{ t('message.login.agreementTips.userAgreement') }}
    </n-button>
    <n-button :text="true" type="primary" @click="handleClickPolicy">
      {{ t('message.login.agreementTips.privacyPolicy') }}
    </n-button>
  </div>
</template>
